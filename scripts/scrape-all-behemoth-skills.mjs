...
const MK_CONFIGS = [
  { mk: 'MK 0', page: 'Companion: Behemoth Zero', section: 'COMPANION ZERO SKILLS', sheetSuffix: 'MK 0 Skills' },
  { mk: 'MK I', page: 'Behemoth MK I', section: 'Behemoth Mk I Skills', sheetSuffix: 'MK I Skills' },
  { mk: 'MK II', page: 'Behemoth MK II', section: 'BEHEMOTH MK II SKILLS', sheetSuffix: 'MK II Skills' },
  { mk: 'MK III', page: 'Companion: Behemoth_MK_III', section: 'BEHEMOTH MK III SKILLS', sheetSuffix: 'MK III Skills' },
  { mk: 'MK IV', page: 'Companion: Behemoth_Mk_IV', section: 'BEHEMOTH MK IV SKILLS', sheetSuffix: 'MK IV Skills' },
];
...
function parseSkillsPage(html, heading) {
  ...
  const section = sectionStart !== -1 && sectionEnd !== -1
    ...
    : (sectionStart !== -1 ? html.substring(sectionStart) : html);
...
async function main() {
  ...
  for (const cfg of MK_CONFIGS) {
    ...
    console.log(`  Parsing ${cfg.mk} skills...`);
    const rows = parseSkillsPage(html, cfg.section);
    console.log(`  ${rows.length} rows`);
    if (rows.length) {
      const ws = XLSX.utils.json_to_sheet(rows);
      XLSX.utils.book_append_sheet(wb, ws, cfg.sheetSuffix);
    }
  }

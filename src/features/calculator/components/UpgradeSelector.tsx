...
export function UpgradeSelector() {
  ...
  const { categories, selectedCategoryId, selectedCategory, selectedGroupName, groupItems, allItems,
    selectCategory, selectGroup, addUpgrade, selectedUpgrades, reset, hasSavedData } = useCalculator();
  ...
  return (
    <Stack spacing={2}>
      <FormControl fullWidth>
        ...
        <Select value={selectedCategoryId ?? ''} label="Category" onChange={e => selectCategory(e.target.value)}>
          {categories.map(cat => (

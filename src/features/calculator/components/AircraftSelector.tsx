...
export function AircraftSelector() {
  ...
  const selectedGroup = useMemo(
    () => AIRCRAFT_GROUPS.find(g => toSlug(g.label) === typeSlug) ?? null,
    [typeSlug],
  );

  const handleSelectGroup = useCallback((label: string) => {
    navigate(`/calculator/aircraft/${toSlug(label)}`);
  }, [navigate]);

  if (!selectedGroup) {
    return (
      <Stack spacing={2}>
        <Typography variant="subtitle2" color="text.secondary">Select aircraft type:</Typography>

...
import { SpacecraftSelector } from './SpacecraftSelector';
...
const SPACECRAFT_GROUP_NAMES = ['Spacecraft', 'Enterprise'];
...
export function SpacecraftPage() {
  ...
  const groupLookup = useMemo(() => buildSlugLookup(SPACECRAFT_GROUP_NAMES), []);
  ...
  useEffect(() => {
    ...
    if (selectedCategoryId === 'spacecraft' && selectedGroupName === groupName) return;
    dispatch({ type: 'HYDRATE_FROM_URL', categoryId: 'spacecraft', groupName: groupName ?? undefined });
  ...
  const breadcrumbs = (
    <Box sx={{ mb: 2 }}>
      <Breadcrumbs aria-label="breadcrumb">
        ...
        {isDetail ? (
          <Link component="button" underline="hover" color="inherit" onClick={() => navigate('/calculator/spacecraft')}>
            Spacecraft
          </Link>
        ) : (
          <Typography color="text.primary">Spacecraft</Typography>
        )}
  ...
  if (!isDetail) {
    return (
      <Card variant="outlined" sx={{ p: 3 }}>
        {breadcrumbs}
        <Typography variant="h5" sx={{ mb: 2 }}>Spacecraft</Typography>
        <SpacecraftSelector />
      </Card>
    );
  }

...
const VEHICLE_GROUP_NAMES = VEHICLE_GROUPS.map(g => g.label);
...
export function VehiclesPage() {
  ...
  const groupLookup = useMemo(() => buildSlugLookup(VEHICLE_GROUP_NAMES), []);
  ...
  useEffect(() => {
    ...
    if (selectedCategoryId === 'vehicles' && selectedGroupName === groupName) return;
    dispatch({ type: 'HYDRATE_FROM_URL', categoryId: 'vehicles', groupName: groupName ?? undefined });
  ...
  const selectedMeta = useMemo(
    () => VEHICLE_GROUPS.find(g => g.label === selectedGroupNameFromUrl) ?? null,
    [selectedGroupNameFromUrl],
  );
